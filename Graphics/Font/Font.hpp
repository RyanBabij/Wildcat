  ...
  //Texture ch [256];
	...
	*/
				...
				unsigned char _colourRed=0, unsigned char _colourGreen=0,unsigned char _colourBlue=0)
	{
		...
		if ( panelWidth < nX || panelHeight < nY || arraySize == 0 )
		{
			return 0;
		}
    ...
    glColor3ub(_colourRed,_colourGreen,_colourBlue);
		...
		if(centeredX==true)
		{
			...
			unsigned int currentX2 = x1;
			/* Figure out how many characters fit on this line. */
			for( unsigned int i=0;i<arraySize  && currentX2+nX <= x2;++i)
			{
				//lineWidth+=nX;
				currentX2+=nX;
			}
		...
		if(centeredY==true)
		{
			int nLines = (arraySize/charsPerLine)+1;
			int vSpace = nLines*(nY+ySpacing)-ySpacing;
        ...
        glColor3ub(_colourRed,_colourGreen,_colourBlue);
				...
				if (currentX+nX > x2 || text[i]=='\n')
				{
					...
					if(centeredX==true)
					{
						unsigned int currentX2 = x1;
						/* Figure out how many characters fit on this line. */
						for( unsigned int i2=i;i2<arraySize && currentX2+nX <= x2;++i2)
						{
							currentX2+=nX;
						}
						int remainderPixels = x2 - currentX2;
				...
				if ( rawTextMode == false && arraySize > 16 && text[i]==' ' )
				{
					int charsUntilSpace = 0;
					for (unsigned int i2=1; i2<20&&i2+i<arraySize;++i2)
					{
						if ( text[i2+i] == ' ' )
						{
							break;
